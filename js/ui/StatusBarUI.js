...
class StatusBarUI extends UIElementBase {
    ...
    /**
     * 获取当前配置（供编辑器使用，exportConfig 的别名）
    ...
    /**
     * 通用更新方法（供编辑器实时预览使用）
     * @param {Object} updates 更新对象，可以是 {x: value} 或 {width: value} 等
     */
    update(updates) {
        // 创建完整配置进行导入
        const currentConfig = this.exportConfig();
        const newConfig = { ...currentConfig, ...updates };
        this.importConfig(newConfig);
    }
    ...
    exportConfig() {
        ...
        return {
            ...
            borderColor: this.config.borderColor || 'transparent',
...
if (typeof window !== 'undefined') {
    window.StatusBarUI = StatusBarUI;
}
